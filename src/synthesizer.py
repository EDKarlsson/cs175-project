...
class Synthesizer:
    ...
    def sampleDistribution(self, pos):
        ...
        if pos == "NN":
            print("Sampling a Noun")
        elif pos == "JJ":
            print("Sampling an Adjective")
        elif pos == "VB":
            print("Sampling a Verb")

    def synthesizeArticle(self, article):
        tagged_article = self.tagArticle(article)
        cleaned_words, removed_count = self.removePOS(tagged_article)
...
if __name__ == "__main__":
    ...
    art = synth.tagArticle(article)
    print(art)

    words = synth.removePOS(art)
    print(words)
